...
build:  ## regenerate index.html from index.src.html
	python3 build.py
...
deploy: build  ## publish to degel.com; MSG="what changed" required
	...
	@git diff --quiet -- index.src.html assets build.py || { echo "ERROR: uncommitted source changes — commit them first"; exit 1; }
	git worktree add .deploy-tmp gh-pages
	cp index.html og.png robots.txt sitemap.xml .deploy-tmp/
